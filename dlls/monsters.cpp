...
void CBaseMonster::SetActivity(Activity NewActivity)
{
    ...
    if (iSequence > ACTIVITY_NOT_AVAILABLE)
    {
        if (pev->sequence != iSequence || !m_fSequenceLoops)
        {
            ...
            if (!(oldActivity == ACT_WALK || oldActivity == ACT_RUN) || !(NewActivity == ACT_WALK || NewActivity ==
                ACT_RUN))
                pev->frame = 0;
        }
...
void CBaseMonster::ReportAIState(void)
{
    ...
    static const char* pStateNames[] = {
        "None", "Idle", "Combat", "Alert", "Hunt", "Prone", "Scripted", "PlayDead", "Dead"
    };
...
BOOL CBaseMonster::FindLateralCover(const Vector& vecThreat, const Vector& vecViewOffset)
{
    ...
    for (i = 0; i < COVER_CHECKS; i++)
    {
        ...
        if (tr.flFraction != 1.0)
        {
            if (FValidateCover(vecLeftTest) && CheckLocalMove(pev->origin, vecLeftTest, NULL, NULL) ==
                LOCALMOVE_VALID)
            {
        ...
        if (tr.flFraction != 1.0)
        {
            if (FValidateCover(vecRightTest) && CheckLocalMove(pev->origin, vecRightTest, NULL, NULL) ==
                LOCALMOVE_VALID)
            {
...
Vector CBaseMonster::ShootAtEnemy(const Vector& shootOrigin)
{
    ...
    {
        return ((m_hEnemy->BodyTarget(shootOrigin) - m_hEnemy->pev->origin) + m_vecEnemyLKP - shootOrigin).
            Normalize();
    }
